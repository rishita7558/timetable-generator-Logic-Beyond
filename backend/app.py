...
def allocate_classrooms_for_timetable(schedule_df, classrooms_df, course_info, semester, branch, section, basket_courses_map=None):
    ...
    def get_suitable_lab_rooms_for_course(course_code):
        """Filter lab rooms based on course type:
           - EC courses use Hardware labs
           - CS and DS courses use Software labs
        ...
        if not available_lab_rooms.empty:
            ...
            # EC courses (Electronics/Electrical) → Hardware labs
            if base_code.startswith('EC'):
                ...
                if not hardware_labs.empty:
                    print(f"         [LAB-TYPE] {base_code} (EC): Using Hardware labs")
                    return hardware_labs
                else:
                    print(f"         [LAB-WARN] {base_code} (EC): No Hardware labs found, using any lab")
                    return available_lab_rooms
            
            # CS and DS courses (Computer Science, Data Science) → Software labs
            elif base_code.startswith(('CS', 'DS')):
                ...
                if not software_labs.empty:
                    print(f"         [LAB-TYPE] {base_code} ({'CS' if base_code.startswith('CS') else 'DS'}): Using Software labs")
                    return software_labs
                else:
                    print(f"         [LAB-WARN] {base_code} ({'CS' if base_code.startswith('CS') else 'DS'}): No Software labs found, using any lab")
                    return available_lab_rooms
    ...
    def allocate_regular_classroom(enrollment_value, day_key, slot_key, is_common_course=False, is_lab_session=False, course_code=None, preferred_capacities_override=None):
        ...
        if is_lab_session:
            # For labs, use appropriate lab rooms based on course type
            if course_code:
        ...
        # For non-lab courses, use regular classrooms
    ...
    # Adjust enrollment based on Common column
    course_enrollment = {}
    for course_code, raw_enrollment in course_enrollment_raw.items():
        info = course_info.get(course_code, {})
        is_common = str(info.get('common', 'No')).strip().upper() == 'YES'
        
        # If common: use full strength; else: use half strength (for single section)
        if is_common:
            course_enrollment[course_code] = raw_enrollment
        else:
            course_enrollment[course_code] = max(1, int(math.ceil(raw_enrollment / 2.0)))

    def compute_effective_enrollment(raw_enrollment, is_common_course):
        """Use full capacity for common courses; split across sections for non-common."""
        try:
        ...
        for time_slot in schedule_df.index:
            ...
            if is_basket_entry and basket_courses_map:
                ...
                if courses_in_basket:
                    ...
                    for course_code in courses_in_basket:
                        ...
                        # Use a key that includes day+time+course+session so:
                        # 1. Same course at same day/time across sections shares the SAME room (within-semester common)
                        # 2. Same course at different day/time gets appropriate room for that slot
                        # 3. Different courses at same day/time get DIFFERENT rooms (cross-semester isolation)
                        common_elective_key = f"ELECTIVE_COMMON_{day}_{time_slot}_{course_code}_{session_type}"
                        ...
                        if existing_common_room:
                            ...
                            if suitable_classroom:
                                # Store as common room for all sections at this specific day/time
                                _GLOBAL_PREFERRED_CLASSROOMS[common_elective_key] = suitable_classroom
                                print(f"         [BASKET-COMMON-NEW] {course_code} ({session_type}) -> {suitable_classroom} (NEW common room for all sections at {day} {time_slot})")
            ...
            if isinstance(course_value, str) and '\n' in course_value:
                ...
                for course_code in courses_in_slot:
                    ...
                    # Use global common room key for electives - include day/time to avoid cross-semester conflicts
                    session_type = 'Tutorial' if is_tutorial else ('Lab' if is_lab else 'Lecture')
                    common_elective_key = f"ELECTIVE_COMMON_{day}_{time_slot}_{clean_code}_{session_type}"
                    ...
                    if existing_common_room:
                        ...
                        if suitable_classroom:
                            # Store as common room for all sections at this specific day/time
                            _GLOBAL_PREFERRED_CLASSROOMS[common_elective_key] = suitable_classroom
                            print(f"        [ESTABLISH-COMMON] {day} {time_slot}: {clean_code} -> {suitable_classroom} (NEW common room for all sections at {day} {time_slot})")
            ...
            if is_basket:
                ...
                if basket_courses_map and basket_name and basket_name in basket_courses_map:
                    ...
                    for idx, individual_course in enumerate(courses_in_basket):
                        ...
                        # Include day/time in the key to avoid cross-semester conflicts at different time slots
                        common_elective_key = f"ELECTIVE_COMMON_{day}_{time_slot}_{individual_course}_{session_type}"
                        ...
                        if not individual_classroom:
                            ...
                            # If successful, store this as the common room for all sections
                            if individual_classroom:
                                _GLOBAL_PREFERRED_CLASSROOMS[common_elective_key] = individual_classroom
...
def find_suitable_classroom_with_tracking(
    ...
    # If this is a lab session, filter to ONLY lab rooms (starting with 'L')
    if is_lab:
